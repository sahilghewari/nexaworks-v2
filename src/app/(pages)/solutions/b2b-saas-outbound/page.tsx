...
export default function B2BSaaSOutboundPage() {
  return (
    <main className="flex-1 bg-[#CBC8BA]">
      {/* Hero Section */}
      <section className="relative overflow-hidden py-20 sm:py-32">
        ...
        <div className="container relative z-10 max-w-4xl text-center">
          ...
          <h1 className="font-display text-4xl font-bold leading-[1.1] text-[#0D1015] sm:text-6xl">
            AI Lead Generation for B2B SaaS
          </h1>
          <p className="mx-auto mt-6 max-w-2xl text-lg text-[#3F3A32] sm:text-xl">
      ...
      {/* Comparison Section */}
      <section className="bg-[#B7B0A0]/30 py-20">
        <div className="container max-w-5xl">
          <h2 className="mb-12 text-center font-display text-3xl font-bold text-[#0D1015]">
            NexaWorks vs. Traditional SDRs
          </h2>
          <div className="grid overflow-hidden rounded-3xl border border-[#0D1015]/10 bg-white/40 shadow-xl md:grid-cols-2">
            <div className="p-8 sm:p-12 border-b md:border-b-0 md:border-r border-[#0D1015]/10">
              <h3 className="text-xl font-bold text-[#0D1015]">Traditional SDR Team</h3>
              <ul className="mt-6 space-y-4">
                {["$85k+ fully loaded cost per rep", "3-6 month ramp time", "Manual list building & scraping", "Generic, templated emails", "High churn rate (14 months)"].map((item) => (
                  <li key={item} className="flex items-start gap-3 text-sm text-[#3F3A32]">
            ...
            <div className="bg-[#A3542B]/5 p-8 sm:p-12">
              <h3 className="text-xl font-bold text-[#A3542B]">NexaWorks AI Engine</h3>
              <ul className="mt-6 space-y-4">
                {["Fraction of the cost of one SDR", "Live and booking meetings in 14 days", "Automated high-intent signal scraping", "Hyper-personalized, human-like sequences", "Never sleeps, never churns"].map((item) => (
                  <li key={item} className="flex items-start gap-3 text-sm font-medium text-[#0D1015]">
                    <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-[#A3542B]" />
                    {item}
                  </li>
                ))}
              </ul>
      ...
      <section className="py-20 sm:py-32">
        <div className="container max-w-3xl text-center">
          <h2 className="font-display text-3xl font-bold text-[#0D1015] sm:text-4xl">
            Ready to scale your pipeline?
          </h2>
          <p className="mt-4 text-lg text-[#3F3A32]">
