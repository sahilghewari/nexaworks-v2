...
const metrics: Array<{ value: number; decimals: number; label: string; prefix?: string; suffix?: string }> = [
  { value: 1250, decimals: 0, label: "QUALIFIED MEETINGS", suffix: "+" },
  { value: 99.2, decimals: 1, suffix: "%", label: "INBOX DELIVERABILITY" },
];
...
export function Hero() {
  ...
  return (
    <section className="relative isolate overflow-hidden bg-[#0A0A0B] mt-[80px]">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,#131316_0%,transparent_55%)]" aria-hidden="true" />
      <div className="absolute inset-x-0 top-[-15%] z-0 h-[420px] bg-gradient-to-b from-[#10B981]/5 via-transparent to-transparent blur-3xl" aria-hidden="true" />
      ...
      >
        {particles.map((particle) => (
          <motion.span
            key={particle.id}
            className="absolute h-1.5 w-1.5 rounded-full bg-[#10B981]/40"
      ...
      <div className="relative z-10 mx-auto grid w-full max-w-6xl gap-8 px-6 py-14 md:grid-cols-[minmax(0,1fr)_minmax(0,0.8fr)] md:items-center md:gap-10 md:py-18">
        ...
        >
          <Badge variant="info" className="w-fit border-none bg-white/5 text-[#A1A1AA]">
            The AI Revenue Engine for B2B SaaS
          </Badge>

          <h1 className="font-display text-4xl font-bold leading-[1.1] text-[#FAFAFA] md:text-6xl md:leading-[1.1]">
            We Build AI Systems That Generate $1M+ in Pipeline
          </h1>
          <p className="max-w-xl text-lg text-[#A1A1AA] md:text-xl">
          ...
          <div className="grid w-full max-w-lg gap-3 rounded-2xl border border-[#27272A] bg-[#131316]/60 p-5 backdrop-blur sm:grid-cols-2">
            {metrics.map((metric) => (
              <MetricCard
                key={metric.label}
                value={metric.value}
                ...
                animate
                className="border-[#27272A] bg-[#1A1A1F] text-[#FAFAFA]"
              />
            ))}
          </div>
        </motion.div>
        ...
        <motion.aside
          className="w-full rounded-3xl border border-[#27272A] bg-[#131316] p-8 shadow-[0_32px_65px_-32px_rgba(0,0,0,0.5)]"
          initial={{ opacity: 0, x: 40 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 0.6, ease: "easeOut", delay: 0.1 }}
        >
            <div className="space-y-5">
              <div className="flex items-center gap-3 text-xs uppercase tracking-[0.35em] text-[#A1A1AA]">
                <span className="inline-flex h-2 w-2 items-center justify-center rounded-full bg-[#10B981]" aria-hidden="true" />
              ...
              <ul className="space-y-3 text-sm text-[#A1A1AA]">
                <li className="flex items-start gap-3">
                  <span className="mt-1 h-2 w-2 rounded-full bg-[#10B981]" aria-hidden="true" />
              ...
              <div className="rounded-2xl border border-[#27272A] bg-[#1A1A1F] p-5 text-sm text-[#A1A1AA]">
                <p className="text-xs uppercase tracking-[0.3em] text-[#10B981]">Pipeline Guarantee</p>
